...
-- Insert default camp settings if not exists
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM camp_settings) THEN
...
BEGIN
  ...
  -- Drop team_switches policies
  DROP POLICY IF EXISTS "Users can view all team switches" ON team_switches;
  DROP POLICY IF EXISTS "Users can insert their own team switches" ON team_switches;
  
  -- Drop camp_settings policies
...
DROP TRIGGER IF EXISTS update_camp_settings_updated_at ON camp_settings;
