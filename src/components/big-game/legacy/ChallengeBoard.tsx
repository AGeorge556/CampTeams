...
const CHALLENGES: Challenge[] = [
  {
    id: 'spirit-chant',
    title: 'Team Spirit Chant',
    description: 'Perform an original team chant in front of everyone.',
    points: 50,
    icon: Zap,
    category: 'team',
  },
  {
    id: 'bible-trivia',
    title: 'Bible Trivia Showdown',
    description: 'Be first to answer 5 consecutive trivia questions.',
    points: 75,
    icon: Star,
    category: 'spiritual',
  },
  {
    id: 'relay-race',
    title: 'Relay Race',
    description: 'Win the team relay race.',
    points: 100,
    icon: Wind,
    category: 'physical',
  },
  {
    id: 'verse-recite',
    title: 'Verse Recitation',
    description: 'Recite the camp verse perfectly from memory.',
    points: 75,
    icon: Target,
    category: 'spiritual',
  },
  {
    id: 'tug-of-war',
    title: 'Tug of War',
    description: 'Win the best-of-three tug of war.',
    points: 150,
    icon: Users,
    category: 'physical',
  },
  {
    ...
    description: 'Receive the most votes for best campfire story.',
    points: 50,
    icon: Flame,
    category: 'creative',
  },
  {
    id: 'talent-show',
    title: 'Talent Show Act',
    description: 'Perform a team talent and earn judge approval.',
    points: 100,
    icon: Award,
    category: 'creative',
  },
  {
    ...
    description: 'Complete the full camp-wide scavenger hunt first.',
    points: 200,
    icon: Target,
    category: 'team',
  },
  {
    id: 'worship-lead',
    title: 'Lead Worship',
    description: 'Have a team member lead a worship moment.',
    points: 75,
    icon: Star,
    category: 'spiritual',
  },
  {
    ...
    description: 'Win the final all-teams showdown event.',
    points: 300,
    icon: Trophy,
    category: 'team',
  },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-orange-500 via-amber-500 to-orange-600 rounded-2xl p-6 text-white shadow-md">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight">
              Camp Challenge Board
            </h1>
            <p className="text-sm text-white/80">
              Compete in challenges to earn points for your team!
            </p>
          </div>
        </div>
        <div className="mt-4 flex items-center gap-3 flex-wrap">
          ...
          <span className="bg-white/20 px-3 py-1 rounded-full text-sm font-semibold">
            {totalPossible.toLocaleString()} pts available
          </span>
          {profile?.is_admin && (
            <span className="bg-white/25 border border-white/30 px-3 py-1 rounded-full text-xs font-semibold">
              Admin: tap team buttons to award points
            </span>
          )}
        </div>
      </div>
            ...
            >
              <div className="p-4">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-0.5">
                      <h3 className="font-bold text-[var(--color-text)] leading-tight text-sm">
                        {challenge.title}
                      </h3>
                ...
                <div className="flex items-center justify-between gap-2">
                  <span
                    className={`text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wide ${CATEGORY_COLORS[challenge.category]}`}
                  >
                    {challenge.category}
                  </span>
                  ...
                  <div className="flex gap-1.5 flex-wrap justify-end">
                    {(
                      Object.entries(TEAMS) as [
                        TeamColor,
                        (typeof TEAMS)[TeamColor],
                      ][]
                    ).map(([teamKey, teamConfig]) => {
                      const done = completedTeams.includes(teamKey);
                      const isLoading =
                        awarding === `${challenge.id}:${teamKey}`;
                      return (
                        <button
                          key={teamKey}
                          onClick={() =>
                            toggleCompletion(challenge.id, teamKey)
                          }
                          disabled={!profile?.is_admin || isLoading}
                          title={
                            profile?.is_admin
                              ? done
                                ? `Remove ${teamConfig.name} completion`
                                : `Award ${teamConfig.name} ${challenge.points} pts`
                              : undefined
                          }
                          ...
                          style={
                              ...
                              ? { background: teamConfig.colorValue }
                              : undefined
                          }
                        >
                          {isLoading ? (
