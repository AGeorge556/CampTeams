...
import { Users, Calendar, Star, Flame, Sun, Cloud, Moon, Snowflake, Leaf, Rainbow, Trees, Mountain, ArrowRight, ArrowLeft } from 'lucide-react'
...
interface WinterCamp2026ContentProps {
  onRegister?: () => void
  onBack?: () => void
}

export default function WinterCamp2026Content({ onRegister, onBack }: WinterCamp2026ContentProps) {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg)] relative overflow-hidden transition-colors duration-300">
      ...
      {/* Main Content */}
      <div className="relative z-10 min-h-screen flex flex-col py-12 px-6">
        {/* Back Button */}
        {onBack && (
          <div className="max-w-4xl mx-auto w-full mb-4">
            <button
              onClick={onBack}
              className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg text-[var(--color-text)] bg-[var(--color-card-bg)] border border-[var(--color-border)] hover:bg-[var(--color-card-bg-hover)] transition-all"
            >
